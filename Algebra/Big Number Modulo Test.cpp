...
f(x1, x2, ..., xn) you have that f(x1, x2, ..., xn) % m
= f*(x1 % m, x2 % m, ..., xn% m) where f* is the function
reinterpreted as working on elements of the ring Zm.
 What this means for you is that if you're asked to compute
 something modulo m and you only use additions/subtractions
 and multiplications then you can just take all intermediate
 calculations modulo m and you'll arrive at the same answer.
Do not use any kind of big arithmetic.
*/
int main()
{
    (x + y) % m = ((x % m) + (y % m)) % m
...
Proof: from Euler's theorem follows that Aphi(MOD) % MOD = 1. It's easy to see that AX * phi(MOD) % MOD = 1 too. So if N = X * phi(MOD) + Y then AN % MOD = AY % MOD and minimal such Y = N % phi(MOD).
Trick #5: (A * B) % MOD
...
Advices: use this trick only if (A % MOD) * (B % MOD) can't be
present in any standart data type because of overflow and you don't
 want to use BigIntegers. But keep in mind that it works in O(logB)
  operations, not in O(1) as (A % MOD) * (B % MOD).
Proof: if B is even then A * B = 2 * A * (B / 2), otherwise A * B =
 A + A * (B - 1).
